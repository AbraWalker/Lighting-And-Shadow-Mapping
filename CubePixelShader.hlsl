...
float4 main(LightingPixelShaderInput input) : SV_Target
{
    ...
    if (dot(gLight1Facing, -light1Direction) > gLight1CosHalfAngle) 
		//**** TODO: This condition needs to be written as the first exercise to get spotlights working
    ...
    float3 finalTextureColour;
    float finalSpecColour;
    float t = sin(radians(360.0f) + wiggle + 0.5);

    finalTextureColour.r = lerp(textureColour2.r, textureColour.r, t);
    finalTextureColour.g = lerp(textureColour2.g, textureColour.g, t);
    finalTextureColour.b = lerp(textureColour2.b, textureColour.b, t);
    finalSpecColour = lerp(textureColour.a, textureColour2.a, t);

    if (t < 0)
    {
        finalTextureColour = textureColour2;
        finalSpecColour = textureColour2.a;
    }
    else if (t > 1)
    {
        finalTextureColour = textureColour;
        finalSpecColour = textureColour.a;
    }

    // Combine lighting with texture colours
        float3 finalColour = diffuseLight * finalTextureColour + specularLight * finalSpecColour;
